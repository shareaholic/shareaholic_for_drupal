...
class ShareaholicHttp {
   ...
   *
   *
   * The options object is an associative array that takes the following
   * options:
   * - method: The http method for the request as a string. Defaults is 'GET'.
   *
   * - headers: The headers to send with the request as an associative array of
   * name/value pairs. Default is empty array.
   *
   * - body: The body to send with the request as an associative array of
   * name/value pairs. Default is NULL. If the body is meant to be parsed as
   * json, specify the content type in the headers option to be
   * 'application/json'.
   *
   * - redirection: The number of redirects to follow for this request as an
   * integer, Default is 5.
   *
   * - timeout: The number of seconds the request should take as an integer.
   * Default is 15 (seconds).
   *
   * - user-agent: The useragent for the request. Default is mozilla browser
   * useragent.
   *
   *
   ...
   * @param bool $ignore_error A flag indicating to log error or not. Default
   *   is false.
   *
   * @return mixed It returns an associative array of name value pairs or false
   *   if there was an error.
   */
  public static function send($url, $options = [], $ignore_error = FALSE) {
  ...
  private static function send_with_drupal($url, $options, $ignore_error) {
    ...
    $request['headers'] = isset($options['headers']) ? $options['headers'] : [];
